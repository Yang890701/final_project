...
const time = ref(8); const fb = ref(null); const done = ref(false); let timer = null;
...
function answer(guess) {
  ...
  fb.value = { correct, real: cur.value.isScam };
  if (correct) { score.value++; streak.value++; best.value = Math.max(best.value, streak.value); }
  else streak.value = 0;
}
function next() {
  fb.value = null;
  if (i.value + 1 >= items.value.length) { done.value = true; clearInterval(timer); return; }
  i.value++; startTimer();
}
...
<template>
  ...
  <div v-else-if="cur">
    <div class="hud"><span>第 {{ i + 1 }}/{{ items.length }}</span><span>🔥 連對 {{ streak }}</span><span>得分 {{ score }}</span></div>
    ...
    <div v-else class="fbk">
      <p :style="{ color: fb.correct ? 'var(--ok)' : 'var(--danger)', fontWeight: 800, fontSize: '1.1rem' }">
        {{ fb.correct ? "答對了！" : "答錯了" }}（正解：{{ fb.real ? "詐騙" : "正常" }}）
      </p>
